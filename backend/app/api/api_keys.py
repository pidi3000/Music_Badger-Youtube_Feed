...
async def list_api_keys(session: DbSession):
    result = await session.execute(select(ApiKey).order_by(ApiKey.group, ApiKey.label))
...
async def create_api_key(body: ApiKeyCreate, session: DbSession):
    key = ApiKey(label=body.label, group=body.group, key_value_encrypted=encrypt(body.key_value))
    session.add(key)
